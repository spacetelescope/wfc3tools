...
def _reprocess_raw_crcorr(raw_file):
    ...
    if asn_tab == "NONE":  # make dummy asn table. remove this clause once calwf3 is patched
        new_asn_tab = np.rec.array(
            [(os.path.basename(raw_file)[0:9], "EXP-DTH", 1)], formats="S14,S14,i1", names="MEMNAME,MEMTYPE,MEMPRSNT"
        )
    ...
    os.rename("temp_" + raw_file.replace("raw", "ima"), "flattened_" + raw_file.replace("raw", "ima"))
...
def make_flattened_ramp_flt(
    ...
    if not sigma_clip:
        if sigma or sigma_upper or sigma_lower or iters:
            true_params = np.array(("sigma", "sigma_upper", "sigma_lower"))[
                np.array((sigma, sigma_upper, sigma_lower)).astype("bool")
            ]
            raise ValueError("sigma_clip = False, but parameters {} were set".format(true_params))
    else:
        if not (sigma):
    ...
    for i in range(ima_hdu[0].header["NSAMP"] - 1):
        avg = _calc_avg(ima_hdu["SCI", i + 1].data[sly, slx], stats_method, sigma_clip, sigma, sigma_upper, sigma_lower, iters)
